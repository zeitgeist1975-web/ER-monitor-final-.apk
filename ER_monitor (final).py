    ...
    <script>const districts = {{ districts|tojson }};
        ...
        async function loadAllHospitals() {
            ...
            try {
                ...
                if (c && c.h && c.h.length && (Date.now() - c.t < 12 * 3600 * 1000)) {
                    allHospitals = c.h; nsdbg('roster sessionStorage n=' + allHospitals.length);
            ...
            try {
                ...
                try { sessionStorage.setItem('allHospitals_v1', JSON.stringify({ t: Date.now(), h: allHospitals })); } catch (e) {}
                nsdbg('roster n=' + allHospitals.length + ' ' + (Date.now() - t0) + 'ms');
                resultsDiv.innerHTML = '';
                applyNameQuery();
    ...
    for q in [sido] + _SIDO_ALIAS.get(sido, []):
        ...
        while page <= 5:
            ...
            got, total = _list_rows_stream(
                _http_get(LIST_API_URL, params=params, timeout=12), sido, 'list ' + q)
            rows.extend(got)
...
def _all_hospitals(force=False):
    """전국 로스터 (캐시 12h). 전국페이징 ∪ 시도별(별칭재시도) 합집합."""
    now = time.time()
    with _ALL_CACHE_LOCK:
        d, ts = _ALL_CACHE['data'], _ALL_CACHE['ts']
    if (not force) and d and (now - ts < _ALL_CACHE_TTL):
        _ns_dbg('roster cache HIT n=%d age=%ds' % (len(d), int(now - ts)))
        return d, [], True
    ...
    for h in rows:
        ...
        seen.add(h['hpid'])
        uniq.append(h)
    uniq.sort(key=lambda x: (x['sido'], x['gugun'], x['name']))

    by_sido = {}
    for h in uniq:
        by_sido[h['sido']] = by_sido.get(h['sido'], 0) + 1
    missing = [k for k in DISTRICTS if by_sido.get(k, 0) == 0]
    if missing:
        _ns_dbg('WARN 로스터 0건 시도: %s' % missing)

    with _ALL_CACHE_LOCK:
        _ALL_CACHE['data'] = uniq
        _ALL_CACHE['ts'] = time.time()
    _ns_dbg('roster DONE n=%d sido=%d fails=%d %.1fs'
            % (len(uniq), len(by_sido), len(fails), time.time() - t0))
...
def api_bed_saturation():
    ...
    t0 = time.time()
    base, base_fails, _ = _all_hospitals(False)
    if not base:
    ...
    try:
        ...
        pass
    _ns_dbg('sat DONE n=%d fails=%d %.1fs' % (len(out), len(fails), time.time() - t0))
    return jsonify({'success': True, 'cached': False, 'queried_at': at,
                    'failed': fails, 'count': len(out), 'rows': out})

# ══ 병원명검색 · 포화도 공용 캐시/디버그 ══
_ALL_CACHE      = {'ts': 0.0, 'data': None}
...
var EX = (function () {
    ...
    async function fetchDetail(hpid, sido, gugun) {
        ...
        for (var i = 0; i < names.length && !detail; i++) {
            try {
                var p = { STAGE1: names[i], pageNo: '1', numOfRows: '400' };
                if (gugun) p.STAGE2 = gugun;
                var doc = await apiGet('getEmrrmRltmUsefulSckbdInfoInqire', p);
                items(doc).forEach(function (it) {
    ...
    return {
        ...
        fetchDetail: fetchDetail, baci: baci,
